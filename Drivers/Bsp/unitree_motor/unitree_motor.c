...
void unitree_send_data(UART_HandleTypeDef *huart,unitree_motor_handle_t *motor, ctrl_param_t ctrl_param) {
    ...
    // SATURATE(ctrl_param.Pos, -411774.0f, 411774.0f);
...
uint8_t unitree_receive_data(UART_HandleTypeDef *huart) {
    ...
    // rx_res = HAL_UART_Receive_DMA(huart, (uint8_t *)&motor_recv_data, sizeof(motor_recv_data));
    if (rx_res == 0) {
        return 1;
    }
...
//     SATURATE(ctrl_param.K_W, 0.0f, 25.599f);
//     SATURATE(ctrl_param.T, -127.99f, 127.99f);
//     // 输出端速度饱和值：电机端804rad/s → 输出端804/6.33≈127rad/s
//     SATURATE(ctrl_param.W, -127.00f, 127.00f);
//     // 输出端位置饱和值：电机端±2π → 输出端±2π/6.33≈±0.992rad
//     SATURATE(ctrl_param.Pos, -0.992f, 0.992f);
...
//     motor_send_data.comd.k_spd = ctrl_param.K_W / 25.6f * 32768.0f;
//     // 输出端位置 → 电机端位置（×减速比）
//     motor_send_data.comd.pos_des = ((ctrl_param.Pos * GEAR_RATIO) + motor->offset_angle) / 6.28318f * 32768.0f;
//     // 输出端速度 → 电机端速度（×减速比）
//     motor_send_data.comd.spd_des = (ctrl_param.W / GEAR_RATIO) / 6.28318f * 256.0f;
//     // 输出端扭矩 → 电机端扭矩（÷减速比）
//     motor_send_data.comd.tor_des = (ctrl_param.T / GEAR_RATIO) * 256.0f;
...
//         p->MError = motor_recv_data.fbk.MError;
//         // 电机端速度 → 输出端速度（÷减速比）
//         p->W = ((float)motor_recv_data.fbk.speed / 256.0f) * 6.28318f / GEAR_RATIO;
//         // 电机端扭矩 → 输出端扭矩（×减速比）
//         p->T = ((float)motor_recv_data.fbk.torque) / 256.0f * GEAR_RATIO;
//         // 电机端位置 → 输出端位置（÷减速比）
//         p->Pos = (6.28318f * ((float)motor_recv_data.fbk.pos) / 32768.0f - p->offset_angle) / GEAR_RATIO;
//         p->footForce = motor_recv_data.fbk.force;
//         p->correct = 1;
