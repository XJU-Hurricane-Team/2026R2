...
int microros_init(void) {
    ...
    rmw_uros_set_custom_transport(
        true, (void *)&huart1, cubemx_transport_open,
    ...
    res |= rcl_init_options_init(&init_options, allocator);
    ...
    rmw_uros_options_set_client_key(base_key + boot_index, rmw_options);

    // init
    res |= rclc_support_init_with_options(&support, 0, NULL, &init_options,
                                          &allocator);
    res |= rclc_node_init_default(&node, "chassis_node", "NULL", &support);
...
void nav_task(void *pvParameters) {
    ...
    TickType_t t_nav = xTaskGetTickCount();
    while (1) {
...
void nav_module_init(void) {
    ...
    rclc_executor_add_subscription(&executor, &subscriber, &nav_pram,
