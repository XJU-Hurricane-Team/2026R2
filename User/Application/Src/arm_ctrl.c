...
#define ARM_TASK_PERIOD_MS          20          // 机械臂控制任务周期 (毫秒)。对应 50Hz 控制频率
...
void robot_arm_set_state_from_microros(uint8_t index);
...
/**
 * @brief 供 micro-ROS 调用的状态切换接口
 * @param index 目标状态索引 (0~4)
 */
void robot_arm_set_state_from_microros(uint8_t index) {
#if ARM_USE_MICROROS_SWITCH
	if (index >= 5U) {
		return;
	}
 ...
 */
static void robot_arm_try_next_state(void) {
	// 当启用 micro-ROS 状态驱动时，READY 阶段要求先收到动态坐标再允许切换。
#if ARM_USE_MICROROS_SWITCH
	if (g_arm_target_index == 1 && g_has_dynamic_target == 0) {
		return;
	}
#endif
