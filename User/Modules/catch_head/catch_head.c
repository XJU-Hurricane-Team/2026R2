...
static dji_motor_handle_t s_push_ball_motor;
static catch_head_dji_joint_t s_push_ball_joint = {
	.motor_handle = &s_push_ball_motor,
...
void catch_head_motor_init(void) {
	if (dji_motor_init(&s_push_ball_motor, DJI_M2006, CAN_Motor1_ID,
					   can1_selected) != 0) {
		s_ready = 0;
		return;
	}
	...
	pid_init(&s_push_ball_joint.speed_pid, 16384, 500, 0, 16384, DELTA_PID, 8, 0.12f, 0.0f);
	pid_init(&s_push_ball_joint.angle_pid, 16384, 1000, 0.2f, 16384, POSITION_PID, 1.0f, 0.0f, 0.0f);
	dm_motor_enable(&s_dm_motor);

	s_push_ball_joint.target_index = 0;
...
void catch_head(void) {
	...
	t_trajectory_update(&s_push_ball_joint.trajectory, &p_des_rad,
					   &w_des_rad_s);
	...
	float add_rpm = pid_calc(
		&s_push_ball_joint.angle_pid, target_out_angle,
		s_push_ball_joint.motor_handle->rotor_degree);

	float final_rpm = (target_out_ff_rpm + add_rpm) *
					  s_push_ball_joint.reduction_ratio;
	float set_current = pid_calc(
		&s_push_ball_joint.speed_pid, final_rpm,
		s_push_ball_joint.motor_handle->speed_rpm);

	s_push_ball_joint.motor_handle->set_value = (int16_t)set_current;

	dji_motor_set_current(can1_selected, DJI_MOTOR_GROUP1,
						  s_push_ball_joint.motor_handle->set_value, 0, 0, 0);
...
void catch_head_on_key(key_press_t key) {
	...
	switch (key) {
		case KEY0_PRESS: {
			catch_head_set_dji_target((uint8_t)((s_push_ball_joint.target_index + 1) % CATCH_HEAD_DJI_TARGET_COUNT));
		} break;
...
void catch_head_set_dji_target(uint8_t target_index) {
	if (s_push_ball_joint.motor_handle == NULL) {
		return;
	}

	target_index %= CATCH_HEAD_DJI_TARGET_COUNT;
	s_push_ball_joint.target_index = target_index;
	t_trajectory_init(&s_push_ball_joint.trajectory,
					 s_push_ball_joint.motor_handle->rotor_degree * CATCH_HEAD_DEG_TO_RAD,
					 s_push_ball_joint.target_angle_deg[target_index] * CATCH_HEAD_DEG_TO_RAD,
					 5.0f, 10.0f, 0.005f);
}
