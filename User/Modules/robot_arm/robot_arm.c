...
static void arm_get_unwrapped_target(RobotArm *arm, float y, float z, float pitch, float out_joints[3])
{
    ...
    while (diff1 > 2.0f * PI) { diff1 -= 2.0f * PI; out_joints[1] -= 2.0f * PI; }
    while (diff1 < -2.0f * PI) { diff1 += 2.0f * PI; out_joints[1] += 2.0f * PI; }
        ...
        case ARM_MOTION_STATE_MULTI_TRANS:
            if (err0 < 0.20f && err1 < 0.20f) {
                ...
                if (arm->multi_trans_index < arm->multi_trans_count) {
                    ...
                    float overshoot_by_layer = ARM_BIG_ARM_FLIP_OVERSHOOT_FORCE_RAD - arm->place_layer * 0.30f;
...
{
    if (arm->big_arm_overshoot_phase == ARM_OVERSHOOT_PHASE_NONE && !arm->big_arm_overshoot_armed) {
        return;
    }
    ...
    if (arm->big_arm_overshoot_phase == ARM_OVERSHOOT_PHASE_OVERSHOOT) {
        ...
        float pos_err = fabsf(arm->damiao_1.position - arm->big_arm_overshoot_joint);
        ...
        if (arm_is_place_like(arm->status)) actual_suction -= ARM_PLACE_SUCTION_OFFSET_RAD;
                             ...
                             float small_speed_base, float small_speed_gain, float small_speed_max)
{
    ...
    uint8_t small_arm_ready = (fabsf(arm->damiao_3.position - joint_des[1]) <= ARM_SMALL_ARM_READY_ERR_RAD);
    uint8_t suction_ready = (fabsf(arm->damiao_4.position - joint_des[2]) <= ARM_SMALL_ARM_READY_ERR_RAD);
    ...
    switch (arm->latch_type) {
        ...
        case ARM_LATCH_PLACE_OVERSHOOT_WAIT:
            {
                uint8_t overshoot_done = (arm->big_arm_overshoot_phase == ARM_OVERSHOOT_PHASE_RECOVER ||
                                          arm->big_arm_overshoot_phase == ARM_OVERSHOOT_PHASE_NONE);
                /* 固定锁死，直到过冲恢复完成 */
                if (overshoot_done) {
        ...
        case ARM_LATCH_SEQ_BIG_ARM_ONLY:
            if (fabsf(arm_wrap_pi(arm->damiao_1.position - arm->takeout_seq_big_arm_target)) <= ARM_TAKEOUT_SEQ_ERR_TOLERANCE_RAD) {
                arm->latch_type = ARM_LATCH_NONE;
                arm->flags.small_arm_wait_locked_inited = 0;
                arm->flags.suction_wait_locked_inited = 0;
            }
        ...
        case ARM_LATCH_SEQ_SMALL_ARM_ONLY:
            if (fabsf(arm->damiao_3.position - arm->takeout_seq_small_arm_target) <= ARM_TAKEOUT_SEQ_ERR_TOLERANCE_RAD) { 
                arm->latch_type = ARM_LATCH_NONE;
                arm->flags.big_arm_wait_locked_inited = 0;
                arm->flags.suction_wait_locked_inited = 0;
            }
    ...
    if (arm->latch_type == ARM_LATCH_SMALL_ARM_WAIT || arm->latch_type == ARM_LATCH_TAKEOUT_WAIT) {
        ...
        *out_joint1_speed = arm_clampf(small_speed_base + small_speed_gain * fabsf(joint_des[1] - arm->damiao_3.position),
        ...
        *out_joint1_cmd = arm->small_arm_wait_locked_pos;
        *out_joint1_speed = ARM_SMALL_WAIT_HOLD_SPEED;
    } else if (arm->latch_type == ARM_LATCH_PLACE_OVERSHOOT_WAIT || arm->latch_type == ARM_LATCH_PLACE_SMALL_SUCTION) {
        /* 放置态小臂吸盘阶段：小臂运动 */
        *out_joint1_cmd = joint_des[1];
        *out_joint1_speed = arm_clampf(small_speed_base + small_speed_gain * fabsf(joint_des[1] - arm->damiao_3.position),
                                        0.04f, small_speed_max);
    ...
    /* 吸盘锁存判断 */
    if (arm->latch_type == ARM_LATCH_SUCTION_WAIT || arm->latch_type == ARM_LATCH_TAKEOUT_WAIT ||
        arm->latch_type == ARM_LATCH_SEQ_BIG_ARM_ONLY || arm->latch_type == ARM_LATCH_SEQ_SMALL_ARM_ONLY ||
        arm->latch_type == ARM_LATCH_PLACE_BIG_RECOVER) {
        if (!arm->flags.suction_wait_locked_inited) {
            /* TAKEOUT_WAIT 使用待取出态解算的目标角度，其他使用电机反馈角度 */
            if (arm->latch_type == ARM_LATCH_TAKEOUT_WAIT) {
                arm->suction_wait_locked_pos = arm->takeout_target_suction_angle;
            } else {
...
void robot_arm_system_init(RobotArm *arm)
{
    ...
    dm_motor_init(&arm->damiao_3, 0x13, 0x03, DM_MODE_PVT, DM_J4340, 6.28f, 10.0f, 10.0f, can3_selected);
    dm_motor_init(&arm->damiao_4, 0x14, 0x04, DM_MODE_PVT, DM_J4310, 6.28f, 10.0f, 10.0f, can3_selected);
...
void robot_arm_set_target(RobotArm *arm, float y, float z, float pitch)
{
    ...
    if (arm_is_place_like(arm->last_status) && !arm_is_place_like(arm->status) &&
        ...
        if (arm->place_layer == 1) {
            /* 第二层: 使用更靠后的过渡点 */
            arm->trans_y[2] = 100.000f; arm->trans_z[2] = 835.030f; arm->trans_pitch[2] = PI/2.0f;
        } else {
            /* 第一层/第三层: 使用更靠前的过渡点 */
 ...
 */
uint8_t robot_arm_start_takeout_sequence(RobotArm *arm, float y, float z, float pitch, float wait_takeout_suction_angle)
{
    float final_joint[3];
    ...
    arm->takeout_seq_big_arm_target = arm->damiao_1.position + dir0 * ARM_TAKEOUT_SEQ_BIG_ARM_STEP_RAD;
    if (arm->takeout_seq_big_arm_target < ARM_BIG_ARM_MIN_ANGLE_RAD) {
        arm->takeout_seq_big_arm_target = ARM_BIG_ARM_MIN_ANGLE_RAD;
    }
    arm->takeout_seq_small_arm_target = arm->damiao_3.position + dir1 * ARM_TAKEOUT_SEQ_SMALL_ARM_STEP_RAD;
...
void robot_arm_update(RobotArm *arm)
{
    ...
    if (arm->motion_state == ARM_MOTION_STATE_TAKEOUT_SEQ_SMALL) {
        ...
        arm->arm_target_z = arm->final_target_z;
        arm->arm_target_pitch = arm->final_target_pitch;
        arm_get_unwrapped_target(arm, arm->final_target_y, arm->final_target_z, arm->final_target_pitch, joint_target);
        arm->motion_state = ARM_MOTION_STATE_DIRECT_MOVE;
    }
...
void arm_apply_ctrl(RobotArm *arm, const float joint_des[3])
{
    ...
    if (arm->latch_type == ARM_LATCH_NONE) {
        if (place_like_mode) {
            ...
            place_speed_min = (fabsf(joint2_err) < ARM_SUCTION_PLACE_FINE_ERR_RAD) ?
                              ARM_SUCTION_PLACE_FINE_SPEED_MIN : ARM_SUCTION_PLACE_SPEED_MIN;

            if ((arm->flip_transition_dir != 0) || (arm->big_arm_overshoot_phase != ARM_OVERSHOOT_PHASE_NONE)) {
            ...
            if ((joint2_err * arm->damiao_4.speed) < 0.0f) {
                joint2_final_speed = fminf(joint2_final_speed, ARM_SUCTION_PLACE_BRAKE_SPEED_MAX);
            }
        } else {
            joint2_final_cmd = joint2_cmd;
